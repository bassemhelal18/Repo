...
class cHoster(iHoster):
    ...
    def __init__(self):
        iHoster.__init__(self, 'vidplay', '-[vidplay]')
...
def decodeVidstream(query):
	...
	ff=requests.get(urlk, headers=hd).text
	if 'status":200' in ff:
...
def encode_id(id_):
	...
	klucze = requests.get('https://raw.githubusercontent.com/matecky/bac/keys/keys.json').json()
	k1 = klucze[0]
	k2 = klucze[1]
	cbn = dec2(k1,id_)
