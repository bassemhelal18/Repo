...
class cHosterGui:
    ...
    def checkHoster(self, sHosterUrl, debrid=True):
        ...
        streamwish = next((x for x in ['streamwish', 'ankrzkz', 'cilootv', 'sfastwish', 'egtpgrvh', 'volvovideo','cimawish',
                                      'wishfast.top', 'fsdcmo.sbs', 'flaswish', 'cdnwish-down', 'heavenlyvideo', 'm3lomatik.store',
                                       'egopxutd', 'obeywish', 'trgsfjller', 'trgsfjll', 'anime4low', 'embedwish', 'cdnwish',
                                        'ma2d.store', 'mohahhda', 'asnwish', 'jodwish'] if x in sHostName), None)
        if streamwish:
        ...
        val = next((x for x in ['goved', 'govad', 'govid.me', 'goveed', 'gov1ad', 'go2ved', 'go1ved', 
                                ...
                                'gon1vd', 'goov9d','goov1d', 'gov9d', 'goov2d', 'g2v3d', 'g1ovd', 'gg1v2d'] if x in sHostName), None)
        if val:
        ...
        val = next((x for x in ['nitroflare', 'tubeload.', 'Facebook', 'fastdrive', 'openload',
                                ...
                                'uploady','mdiaload', 'usersdrive', '1cloudfile'] if x in sHostName), None)
        if val:
            return False
